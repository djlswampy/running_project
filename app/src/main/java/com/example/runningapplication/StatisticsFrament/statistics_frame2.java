...
public class statistics_frame2 extends Fragment { //월간
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.activity_statistics_frame2,container, false);
        ...
        //이번 달의 데이터 구하는 코드
        if(user.year_month_KeyList.get(today.substring(0, 6)) != null){

            for (String key: user.year_month_KeyList.get(today.substring(0, 6))) { // year_month_week_KeyList의 key가 연,월,주 && value자체가 totalRecordList의 key리스트

                year_month_time += user.totalRecordList.get(key)[0]; //밀리초
                year_month_distance += user.totalRecordList.get(key)[1]; // km

            };

            Seconds = (int) (year_month_time / 1000); //밀리초를 초단위로
            Minutes = Seconds / 60;
            Seconds = Seconds % 60;
            String thisMonthDistance = String.format("%.2f", year_month_distance);


            //평군페이스 계산. 초단위
            int averagePace_Sec= (int)((year_month_time / 1000) / year_month_distance);
        ...
        calendar.add(Calendar.MONTH, -1); //지난 달
        ...
        //지난 달의 데이터 구하는 코드
        if(user.year_month_KeyList.get(lastMonth.substring(0, 6)) != null){
            System.out.println("확인확인확인@@!@#!@#@@@@@@@@ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ" + user.year_month_KeyList.get(lastMonth.substring(0, 6)));

            for (String key: user.year_month_KeyList.get(lastMonth.substring(0, 6))) { // year_month_week_KeyList의 key가 연,월,주 && value자체가 totalRecordList의 key리스트
