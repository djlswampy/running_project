...
public class statistics_frame1 extends Fragment { //주간
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.activity_statistics_frame1,container, false);
        ...
        //이번주의 데이터 구하는 코드
        if(user.year_month_week_KeyList.get(today.substring(0, 7)) != null){
            for (String key: user.year_month_week_KeyList.get(today.substring(0, 7))) { // year_month_week_KeyList의 key가 연,월,주 && value자체가 totalRecordList의 key리스트

                year_month_week_time += user.totalRecordList.get(key)[0]; //밀리초
                year_month_week_distance += user.totalRecordList.get(key)[1]; // km

            };

            Seconds = (int) (year_month_week_time / 1000); //밀리초를 초단위로
            Minutes = Seconds / 60;
            Seconds = Seconds % 60;
            ...
            //평군페이스 계산. 초단위
            int averagePace_Sec= (int)((year_month_week_time / 1000) / year_month_week_distance);
            ...
            tv_this_distance.setText(thisWeekDistance + "km");
            tv_this_time.setText("" + Minutes + ":"
                    + String.format("%02d", Seconds));

            year_month_week_time = 0; //초기화
            year_month_week_distance =0;
...
//--------------------
        ...
        calendar.add(Calendar.DATE, -7); //지난주
        Date oneWeekAgoDate = calendar.getTime();

        String lastWeek = new SimpleDateFormat("yyyyMMWddHHmmss").format(oneWeekAgoDate);

        //지난 주 데이터
        if(user.year_month_week_KeyList.get(lastWeek.substring(0, 7)) != null){
            for (String key: user.year_month_week_KeyList.get(today.substring(0, 7))) { // year_month_week_KeyList의 key가 연,월,주 && value자체가 totalRecordList의 key리스트
            ...
            tv_last_distance.setText(thisWeekDistance + "km");
